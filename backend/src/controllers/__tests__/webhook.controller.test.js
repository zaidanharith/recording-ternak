const { getFarmerNameByPhone } = require('../../repositories/farmer.repository');
...
describe('handleWebhookEvent unregistered sender gate', () => {
  it('replies with the unregistered-sender message and skips handleMessage when the sender is not a registered farmer', async () => {
    getFarmerNameByPhone.mockResolvedValue(null);
    ...
    expect(getFarmerNameByPhone).toHaveBeenCalledWith('628123');
  ...
  it('dispatches to handleMessage using the name registered in the database, not the WhatsApp profile name', async () => {
    getFarmerNameByPhone.mockResolvedValue({ name: 'Pak Budi Santoso' });
    ...
    expect(handleMessage).toHaveBeenCalledWith('kambing 12 beranak 2 ekor', '628123', 'Pak Budi Santoso');
